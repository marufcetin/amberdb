...
* **`1` or `'index'` (Hybrid Index-Only Acceleration):** Secondary index files (`.inx`, `.src`, `.fld`, `.fac`, `.unq`, `.slg`) are placed on RAM-disk and synchronously dual-written to physical disk. Master record data (`.db`) remains on physical disk. Searches, filtering, and lookups run at memory speeds with minimal RAM footprint and indexes survive reboots.
...
* **`4` or `'async'` (Asynchronous Write-Behind):** All reads and writes are served from RAM-disk at microsecond speeds. Writes to permanent disk are deferred and tracked as dirty events in `dbstore/journal/sync_ramdisk`. A background sync worker (`amberdb_daemon.pl`) flushes changes to disk periodically. *Note: During an active transaction (`transact_start`), operations automatically elevate to synchronous dual-write mode.*
