...
`dump()` creates a compressed, portable `.amberdb` native database archive containing table schemas (`schema/*.table`, `schema/*.dbase`), authoritative master data tables (`tables/*.db`, `*.del`, `*.aut`, `*.cnt`, `*.str`), and a cryptographically verified SHA-256 `manifest.json`. Derived secondary indexes are excluded to save space.
