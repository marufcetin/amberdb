...
* **`1` veya `'index'` (Hibrit İndeks Hızlandırması):** Yalnızca ikincil indeks dosyaları (`.inx`, `.src`, `.fld`, `.fac`, `.unq`, `.slg`) RAM-diske alınır ve kalıcı diske senkron çift yazılır. Ana veri (`.db`) kalıcı diskte saklanır. Arama, filtreleme ve sıralama bellek hızında çalışırken RAM tüketimi minimum düzeyde tutulur ve indeks dayanıklılığı korunur.
...
* **`4` veya `'async'` (Asenkron Diske Yazma - Write-Behind):** Tüm okuma ve yazmalar RAM-diskte mikrosaniye hızında gerçekleştirilir. Diske yazma ertelenir ve `dbstore/journal/sync_ramdisk` günlüğüne dirty olayı fırlatılır. Tekil bir arka plan daemon'u (`amberdb_daemon.pl`) periyodik olarak diske yazar. *Önemli: `transact_start` işlemi başladığında tüm tablolar ve indeksler istisnasız senkron dual-write moduna geçer.*
