...
subtest '1. .str dictionary prefix architecture (s: and n:)' => sub {
    ...
    my $tinfo = $dbp->table_info('catalog_product');

    # Convert strings to IDs (write mode)
    my @ids = $dbp->field_to_list( 'Elektronik', 'write', $table_path, $tinfo, 1 );
    ...
    my @ids2 = $dbp->field_to_list( 'Elektronik', 'write', $table_path, $tinfo, 1 );
...
subtest '2. Columnar .fac files and active-only indexing' => sub {
    ...
    $dbp->insert_id( 'catalog_product', 101, 'Telefon', 'Apple', 'Siyah', 1 );
    $dbp->insert_id( 'catalog_product', 102, 'Telefon', 'Samsung', 'Beyaz', 1 );
    $dbp->insert_id( 'catalog_product', 103, 'Bilgisayar', 'Apple', 'Gri', 1 );

    # Inactive product ($status == 0) -> should NOT be indexed in .fac
    $dbp->insert_id( 'catalog_product', 104, 'Telefon', 'Apple', 'Mavi', 0 );
...
subtest '3. Dynamic Scope (base_ids) bypassing full scan' => sub {
    ...
    my $scoped_brands = $dbp->field_fltkeys(
        'catalog_product',
        {
            target_block => 2,
            base_ids     => [ 101, 102 ],
...
subtest '4. facet_menu generation with selected filters' => sub {
    ...
    my $facet_menu = $dbp->facet_menu(
        'catalog_product',
        { 1 => 'Telefon' }, # Filter by Telefon
        $dbp->table_info('catalog_product')->{facet_block}
    );
...
subtest '5. Facet lifecycle with Junk transitions (Active -> Junk -> Active)' => sub {
    ...
    $dbp->{_table}->{catalog_product}->{use_junk}   = 1;
    $dbp->{_table}->{catalog_product}->{junk_rules} = [ [ 4, 'ne', 1 ] ]; # block 4 != 1 -> junk

    my $table_path = $dbp->table_path('catalog_product');

    # Initial state: 101 is active (status 1) -> in .fac
    my ($fac_101_init) = $dbp->index_get( "${table_path}_1.fac", 101, 'raw' );
    ...
    $dbp->modify_id( 'catalog_product', 101, 'Telefon', 'Apple', 'Siyah', 0 );
    my ($fac_101_junk) = $dbp->index_get( "${table_path}_1.fac", 101, 'raw' );
    ok( !defined $fac_101_junk, "Active -> Junk: 101 was removed from .fac" );

    # Verify 101 is in junk .jinx
    my ( undef, @jinx_keys ) = $dbp->index_get( "$table_path.jinx", "keys" );
    ...
    $dbp->modify_id( 'catalog_product', 101, 'Telefon', 'Apple', 'Siyah', 1 );
    my ($fac_101_restored) = $dbp->index_get( "${table_path}_1.fac", 101, 'raw' );
