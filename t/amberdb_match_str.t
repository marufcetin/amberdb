...
subtest 'field_to_list normalization and trim_space' => sub {
    ...
    # 1. Undef / empty
    is_deeply( [ $dbp->field_to_list(undef) ], [], 'undef returns empty list' );
    is_deeply( [ $dbp->field_to_list('') ],    [], 'empty string returns empty list' );
    is_deeply( [ $dbp->field_to_list('   ') ], [], 'whitespace-only returns empty list' );

    # 2. Single value with whitespace
    is_deeply( [ $dbp->field_to_list('  Edebiyat  ') ], ['Edebiyat'], 'single string trimmed' );

    # 3. Comma / semicolon delimited with multiple spaces & tabs
    my $str1 = " Edebiyat ,  Dünya   Klasikleri ; \t Rus Romanları \n ";
    is_deeply(
        [ $dbp->field_to_list($str1) ],
...
SCHEMA
    ...
    my $id1 = $dbp->insert_id( 'products', 1, '49, 112', 'Kitap 1' );
    my $id2 = $dbp->insert_id( 'products', 2, '112, 167', 'Kitap 2' );
    my $id3 = $dbp->insert_id( 'products', 3, '49', 'Kitap 3' );

    my $fld_path = $dbp->table_path('products') . '_1.fld';
    ok( -e $fld_path, 'products_1.fld index file exists' );

    # Verify .fld contains numeric keys 49, 112, 167
    my ( undef, @recs_49 )  = $dbp->index_get( $fld_path, '49' );
    my ( undef, @recs_112 ) = $dbp->index_get( $fld_path, '112' );
    my ( undef, @recs_167 ) = $dbp->index_get( $fld_path, '167' );
    ...
    # field_fetch queries
    my @fetch_49 = $dbp->field_fetch( 'products', 1, 49 );
    is( scalar(@fetch_49), 2, 'field_fetch(1, 49) returns 2 records' );

    my @fetch_both = $dbp->field_fetch( 'products', 1, '49, 167' );
...
subtest 'non-rdbm string match_block with .str and lastid (Case 2)' => sub {
    ...
    my $schema_file = File::Spec->catfile( $scheme_dir, 'tags.table' );
    open my $fh, '>', $schema_file or die "Cannot create schema: $!";
    print $fh <<'SCHEMA';
{
    id_type      => 'num',
...
SCHEMA
    ...
    my $id1 = $dbp->insert_id( 'tags', 1, 'Edebiyat, Dünya Klasikleri, Rus Romanları', 'Makale 1' );
    my $id2 = $dbp->insert_id( 'tags', 2, 'Dünya Klasikleri, Bilim Kurgu', 'Makale 2' );

    my $str_path = $dbp->table_path('tags') . '_1.str';
    my $fld_path = $dbp->table_path('tags') . '_1.fld';
    ...
    my ($lastid)  = $dbp->index_get( $str_path, 'lastid', 'raw' );
    my ($id_edeb) = $dbp->index_get( $str_path, 'Edebiyat', 'raw' );
    my ($id_dunya)= $dbp->index_get( $str_path, 'Dünya Klasikleri', 'raw' );
    my ($id_rus)  = $dbp->index_get( $str_path, 'Rus Romanları', 'raw' );
    my ($id_bilim)= $dbp->index_get( $str_path, 'Bilim Kurgu', 'raw' );
    ...
    my ( undef, @fld_1 ) = $dbp->index_get( $fld_path, '1' );
    my ( undef, @fld_2 ) = $dbp->index_get( $fld_path, '2' );
...
subtest 'modify_id, delete_id and index rebuild' => sub {
    ...
    my $schema_file = File::Spec->catfile( $scheme_dir, 'news.table' );
    open my $fh, '>', $schema_file or die "Cannot create schema: $!";
    print $fh <<'SCHEMA';
{
    id_type      => 'num',
...
SCHEMA
    ...
    $dbp->insert_id( 'news', 1, 'Teknoloji, Yapay Zeka', 'Haber 1' );
    $dbp->insert_id( 'news', 2, 'Yapay Zeka, Robotik',   'Haber 2' );

    my $str_path = $dbp->table_path('news') . '_1.str';
    my ($lastid) = $dbp->index_get( $str_path, 'lastid', 'raw' );
    is( $lastid, 3, 'initial lastid is 3' );

    # Modify news 1: replace 'Teknoloji' with new topic 'Uzay'
    $dbp->modify_id( 'news', 1, 'Uzay, Yapay Zeka', 'Haber 1' );

    my ($new_lastid) = $dbp->index_get( $str_path, 'lastid', 'raw' );
    my ($id_uzay)    = $dbp->index_get( $str_path, 'Uzay', 'raw' );
    ...
    my @fetch_tekno = $dbp->field_fetch( 'news', 1, 'Teknoloji' );
    is( scalar(@fetch_tekno), 0, 'Teknoloji no longer matches news 1' );

    # Querying 'Uzay' returns news 1
    my @fetch_uzay = $dbp->field_fetch( 'news', 1, 'Uzay' );
    is( scalar(@fetch_uzay), 1, 'Uzay matches news 1' );

    # Delete news 2
    $dbp->delete_id( 'news', 2 );
    my @fetch_robo = $dbp->field_fetch( 'news', 1, 'Robotik' );
    ...
    my @all_records = $dbp->read_all( 'news', 0, 0, no_index => 1 );
    $tools->set_fields( 'news', @all_records );

    my @fetch_uzay_rebuilt = $dbp->field_fetch( 'news', 1, 'Uzay' );
...
subtest 'batch match_add handle lifecycle and whitespace cleaning' => sub {
    ...
    my $schema_file = File::Spec->catfile( $scheme_dir, 'batch_test.table' );
    open my $fh, '>', $schema_file or die "Cannot create schema: $!";
    print $fh <<'SCHEMA';
{
    id_type      => 'num',
...
SCHEMA
    ...
    my @f_list = $dbp->field_to_list("  Bilim\nKurgu  ,  Yapay\tZeka  ");
    ...
    my $table_info = $dbp->table_info('batch_test');

    # Run match_add for the whole batch
    $dbp->match_add( $table_path, $table_info, \@batch );
    ...
    my ($lastid)   = $dbp->index_get( $str_path, 'lastid', 'raw' );
    my ($id_fizik) = $dbp->index_get( $str_path, 'Fizik', 'raw' );
    my ($id_kimya) = $dbp->index_get( $str_path, 'Kimya', 'raw' );
    ...
    my ( undef, @recs_fizik ) = $dbp->index_get( $fld_path, $id_fizik );
    ...
    my @read_ids = $dbp->field_to_list( 'Fizik, Geometri', 'read', $table_path, $table_info, 1 );
