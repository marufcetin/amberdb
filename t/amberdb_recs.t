...
subtest 'CRUD & recs_* pipeline' => sub {
    ...
    my $table_path = $dbp->table_path('test_recs');
    my $file_path  = "$table_path.$dbp->{db_ext}";

    # 1. Insert records via insert_id
    my $id1 = $dbp->insert_id( 'test_recs', 1, 'Ahmet', 'Istanbul', 'ahmet@example.com' );
    my $id2 = $dbp->insert_id( 'test_recs', 2, 'Mehmet', 'Ankara', 'mehmet@example.com' );
    my $id3 = $dbp->insert_id( 'test_recs', 3, 'Ayse', 'Izmir', 'ayse@example.com' );
    ...
    ok( $dbp->exist_id( 'test_recs', 1 ), 'exist_id returns 1 for existing ID 1' );
    ok( !$dbp->exist_id( 'test_recs', 999 ), 'exist_id returns 0 for non-existent ID 999' );

    my $exist_res = $dbp->exist_list( 'test_recs', 1, 3, 500 );
    ...
    ok( $dbp->recs_exist( $file_path, 1 ), 'recs_exist auto-opens file and returns 1 for existing ID 1' );
    ok( $dbp->recs_exist( $file_path, 2 ), 'recs_exist returns 1 for existing ID 2' );
    ok( !$dbp->recs_exist( $file_path, 999 ), 'recs_exist returns 0 for non-existent ID 999' );

    # 4. Test recs_exist on open file (multiple keys)
    my $map = $dbp->recs_exist( $file_path, 1, 2, 999 );
    is_deeply( $map, { 1 => 1, 2 => 1, 999 => 0 }, 'recs_exist multi-key map correct' );

    # 5. Test recs_keys on open file
    my @keys = sort { $a <=> $b } $dbp->recs_keys($file_path);
    ...
    my $all_data = $dbp->recs_scan($file_path);
    ...
    $dbp->recs_del( $file_path, 2 );
    ok( !$dbp->recs_exist( $file_path, 2 ), 'recs_exist returns 0 after recs_del' );

    my @remaining_keys = sort { $a <=> $b } $dbp->recs_keys($file_path);
...
subtest 'Pre-opened table handle with recs_*' => sub {
    ...
    my $handle = $dbp->table_write($file_path);
    ok( $handle, 'Table opened explicitly with table_write' );

    # Perform recs_* operations on open handle
    $dbp->recs_put( $file_path, [ 4, 'Fatma', 'Bursa', 'fatma@example.com' ] );
    ok( $dbp->recs_exist( $file_path, 4 ), 'recs_exist works on already open table' );

    my $get_res = $dbp->recs_get( $file_path, 4 );
    ok( $get_res && $get_res->{4}, 'recs_get works on already open table' );

    my @keys = sort { $a <=> $b } $dbp->recs_keys($file_path);
    is_deeply( \@keys, [ 1, 3, 4 ], 'recs_keys works on already open table' );

    $dbp->recs_del( $file_path, 4 );
    ok( !$dbp->recs_exist( $file_path, 4 ), 'recs_del works on already open table' );

    # Explicit close
    ok( $dbp->table_close($file_path), 'table_close succeeds after recs_* operations' );
};
