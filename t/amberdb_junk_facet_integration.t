...
subtest '1. Insert active product (status=1)' => sub {
    ...
    my ($fac_101)           = $adb->index_get( "${tpath}_1.fac", 101, 'raw' );
...
subtest '2. Transition Active -> Junk via modify_id (status=0)' => sub {
    ...
    my ($fac_101_after)       = $adb->index_get( "${tpath}_1.fac", 101, 'raw' );
...
subtest '3. Transition Junk -> Active via modify_id (status=1)' => sub {
    ...
    my ($fac_101_back)       = $adb->index_get( "${tpath}_1.fac", 101, 'raw' );
...
subtest '4. Insert product initially as Junk (status=0)' => sub {
    ...
    my ($fac_102)           = $adb->index_get( "${tpath}_1.fac", 102, 'raw' );
...
subtest '5. Delete product in Junk tier (status=0)' => sub {
    ...
    my ( undef, @jinx_after_del ) = $adb->index_get( "$tpath.jinx", "keys" );
    my ($fac_102_after_del)       = $adb->index_get( "${tpath}_1.fac", 102, 'raw' );
...
subtest '6. Standalone Junk without Facet (use_junk=1, use_facet=0)' => sub {
    ...
    my ( undef, @jinx_keys ) = $adb->index_get( "$jpath.jinx", "keys" );
...
subtest '7. Standalone Facet without Junk (use_junk=0, use_facet=1)' => sub {
    ...
    my ($f1)                 = $adb->index_get( "${fpath}_1.fac", 1, 'raw' );
    my ($f2)                 = $adb->index_get( "${fpath}_1.fac", 2, 'raw' );
...
subtest '8. Differing junk_rules and facet_rules' => sub {
    ...
    my ( undef, @jinx_keys ) = $adb->index_get( "$dpath.jinx", "keys" );
    my ( undef, @fac_acts )  = $adb->index_get( "$dpath.fac", "active" );
    my ($fac_1)              = $adb->index_get( "${dpath}_1.fac", 1, 'raw' );
    my ($fac_2)              = $adb->index_get( "${dpath}_1.fac", 2, 'raw' );
    my ($fac_3)              = $adb->index_get( "${dpath}_1.fac", 3, 'raw' );
...
subtest '9. Bulk insert_list and delete_list with Junk and Facet' => sub {
    ...
    my ( undef, @jinx_keys ) = $adb->index_get( "$tpath.jinx", "keys" );
