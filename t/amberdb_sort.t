...
subtest 'Sort Key Normalization' => sub {
    plan tests => 15;
    my $dbp = AmberDB->new( cfg => { language => 'tr' } );

    # 1. Number normalization (with offset 1e12)
    is( $dbp->normalize_sort_key( 0, 'num' ), '1000000000000.000000', 'Zero numeric normalization' );
    is( $dbp->normalize_sort_key( 150.5, 'num' ), '1000000000150.500000', 'Positive float normalization' );
    is( $dbp->normalize_sort_key( 100, 'num' ), '1000000000100.000000', 'Integer numeric normalization' );
    is( $dbp->normalize_sort_key( -500, 'num' ), '0999999999500.000000', 'Negative number normalization' );
    is( $dbp->normalize_sort_key( -150.75, 'num' ), '0999999999849.250000', 'Negative decimal float normalization' );

    # Compare numeric string ordering: -500 < -150.75 < 0 < 99.99 < 150.5
    my $norm_neg500 = $dbp->normalize_sort_key( -500, 'num' );
    my $norm_neg150 = $dbp->normalize_sort_key( -150.75, 'num' );
    my $norm_zero   = $dbp->normalize_sort_key( 0, 'num' );
    my $norm_pos99  = $dbp->normalize_sort_key( 99.99, 'num' );
    my $norm_pos150 = $dbp->normalize_sort_key( 150.5, 'num' );
    ...
    is( $dbp->normalize_sort_key( 'Buzdolabı NoFrost', 'string', 8 ), 'buzdolab', '8-byte truncation truncates long string to 8 characters' );
    is( $dbp->normalize_sort_key( 'Çamaşır Makinesi', 'string', 8 ), 'camasirm', '8-byte Turkish ASCII folding and truncation' );
    is( $dbp->normalize_sort_key( 'Kısa', 'string', 8 ), 'kisa    ', 'Shorter string padded with spaces to 8 bytes' );

    # 3. Reference Protection
    is( $dbp->normalize_sort_key( ['FirstItem', 'SecondItem'], 'string', 16 ), 'firstitem       ', 'ARRAY ref unwraps first element' );
    is( $dbp->normalize_sort_key( { key => 'val' }, 'string', 16 ), '                ', 'HASH ref returns padded empty string' );
    is( $dbp->normalize_sort_key( undef, 'string', 16 ), '                ', 'Undef returns padded empty string' );
};
...
SCHEMA
    ...
    $dbp->insert_id( 'products', 101, 'Laptop', 1500, '2026-01-01 10:00:00' );
    $dbp->insert_id( 'products', 102, 'Mouse',  25,   '2026-01-02 11:00:00' );
    $dbp->insert_id( 'products', 103, 'Monitor', 300,  '2026-01-03 12:00:00' );
    $dbp->insert_id( 'products', 104, 'Keyboard', 75,  '2026-01-04 13:00:00' );
    ...
    my @recs_default = $dbp->read_all( 'products', sort => { blk => 2 } );
    ...
    my @recs_rev = $dbp->read_all( 'products', sort => { blk => 2, reverse => 1 } );
    ...
    my @recs_num = $dbp->read_all( 'products', sort => 2 );
    is_deeply( [ map { $_->[0] } @recs_num ], [ 101, 103, 104, 102 ], 'sort => 2 defaults to sondan başa / DESC' );
    my @recs_neg = $dbp->read_all( 'products', sort => -2 );
    ...
    my @recs_asc = $dbp->read_all( 'products', sort => { blk => 2, dir => 'asc' } );
    ...
    my ( $page_cnt, @page_recs ) = $dbp->read_all( 'products', 1, 2, sort => { blk => 2, reverse => 1 } );
    ...
    $dbp->modify_id( 'products', 102, 'Mouse Ultra', 2000, '2026-01-02 11:00:00' );
    my @mod_recs = $dbp->read_all( 'products', sort => { blk => 2, dir => 'asc' } );
    ...
    $dbp->delete_id( 'products', 103 );
    my @del_recs = $dbp->read_all( 'products', sort => { blk => 2, dir => 'asc' } );
    ...
    $dbp->insert_list( 'products', @bulk );
    my @bulk_recs = $dbp->read_all( 'products', sort => { blk => 2, dir => 'asc' } );
    ...
    my @name_recs = $dbp->read_all( 'products', sort => { blk => 1, dir => 'asc' } );
    ...
    my @reindex_recs = $dbp->read_all( 'products', sort => { blk => 2, dir => 'asc' } );
...
SCHEMA
    ...
    $dbp->insert_id( 'items', 201, 'Red T-Shirt',   15, 'clothing' );
    $dbp->insert_id( 'items', 202, 'Blue T-Shirt',  45, 'clothing' );
    $dbp->insert_id( 'items', 203, 'Green T-Shirt', 25, 'clothing' );
    $dbp->insert_id( 'items', 204, 'Red Pants',     60, 'clothing' );

    # 1. field_fetch default sort (sondan başa / desc) by price (block 2)
    my @ff_def = $dbp->field_fetch( 'items', 3, 'clothing', sort => { blk => 2 } );
    ...
    my @ff_rev = $dbp->field_fetch( 'items', 3, 'clothing', sort => { blk => 2, reverse => 1 } );
    ...
    my @ff_id_rev = $dbp->field_fetch( 'items', 3, 'clothing', sort => { reverse => 1 } );
    ...
    my ( undef, @ff_page ) = $dbp->field_fetch( 'items', 3, 'clothing', 0, 2, sort => { blk => 2, reverse => 1 } );
    ...
    my @st_def = $dbp->search_table( 'items', 't-shirt', sort => { blk => 2 } );
    ...
    my @st_rev = $dbp->search_table( 'items', 't-shirt', sort => { blk => 2, reverse => 1 } );
    ...
    my @st_id_rev = $dbp->search_table( 'items', 't-shirt', sort => { reverse => 1 } );
    ...
    my @ff_name_asc = $dbp->field_fetch( 'items', 3, 'clothing', sort => { blk => 1, reverse => 1 } );
    is_deeply( [ map { $_->[0] } @ff_name_asc ], [ 202, 203, 204, 201 ], 'field_fetch sort by Name (blk 1) ASC [202, 203, 204, 201]' );

    my @st_name_asc = $dbp->search_table( 'items', 't-shirt', sort => { blk => 1, reverse => 1 } );
    ...
    $dbp->modify_id( 'items', 201, 'Red T-Shirt Exclusive', 100, 'clothing' );
    my @st_mod_price = $dbp->search_table( 'items', 't-shirt', sort => { blk => 2 } ); # DESC
    is_deeply( [ map { $_->[0] } @st_mod_price ], [ 201, 202, 203 ], 'After modify_id: 201 moved to highest price in search_table DESC' );

    my @ff_mod_price = $dbp->field_fetch( 'items', 3, 'clothing', sort => { blk => 2, reverse => 1 } ); # ASC
    ...
    $dbp->delete_id( 'items', 202 );
    my @st_del_price = $dbp->search_table( 'items', 't-shirt', sort => { blk => 2 } );
    is_deeply( [ map { $_->[0] } @st_del_price ], [ 201, 203 ], 'After delete_id: 202 removed from search_table results' );

    my @ff_del_price = $dbp->field_fetch( 'items', 3, 'clothing', sort => { blk => 2, reverse => 1 } );
    ...
    $dbp->insert_id( 'items', 205, 'Yellow T-Shirt', 5, 'clothing' );
    my @st_ins_price = $dbp->search_table( 'items', 't-shirt', sort => { blk => 2, reverse => 1 } );
...
subtest 'Primary Key Binary Index (.inx) O(1) Seeking' => sub {
    ...
    my $schema_file = File::Spec->catfile( $scheme_dir, 'orders.table' );
    open my $fh, '>', $schema_file or die "Cannot create schema file: $!";
    print $fh <<'SCHEMA';
{
    id_type      => 'num',
...
SCHEMA
    ...
    for my $id ( 1 .. 50 ) {
        $dbp->insert_id( 'orders', $id, "Order #$id", $id * 10 );
    }

    my $table_path = $dbp->table_path('orders');
    ok( -e "$table_path.inx", '.inx binary primary key sequence file created' );

    # Page 1: start = 0, limit = 5
    my ( $cnt1, @p1 ) = $dbp->read_all( 'orders', 0, 5 );
    ...
    my ( undef, @p3 ) = $dbp->read_all( 'orders', 10, 5 );
    is_deeply( [ map { $_->[0] } @p3 ], [ 11 .. 15 ], 'Middle page (11..15) sliced via bin_decode on .inx' );
};

subtest 'convert_all_tables Batch Conversion' => sub {
    plan tests => 2;
    my $temp_dir = tempdir( CLEANUP => 1 );
    my $conf_dir = File::Spec->catdir( $temp_dir, 'conf' );
    my $scheme_dir = File::Spec->catdir( $temp_dir, 'scheme' );
...
SCHEMA
    ...
    $dbp->insert_id( 'products', 1, 'laptop', 'elektronik' );
    $dbp->insert_id( 'products', 2, 'telefon', 'elektronik' );

    my $tools = AmberDB::Tools->new($dbp);
    my $converted = $tools->convert_all_tables();
    ok( $converted->{products}, 'convert_all_tables processed products table' );
